// node_topology_host.zig — Node topology panel draw functions.
...
pub fn drawNodeTopologyPanel(self: anytype, _manager: anytype, rect: anytype) void {
    ...
    const table_btn_x = rect.max[0] - inset - btn_w * 2.0 - btn_gap;
    const tree_btn_x = rect.max[0] - inset - btn_w;
    ...
    if (self.ws.node_topology_table_view) {
        self.drawNodeTableView(content_rect);
    } else {
        self.drawNodeTreeView(content_rect);
    }
}

pub fn drawNodeTableView(self: anytype, rect: anytype) void {
    const layout = self.panelLayoutMetrics();
    const inset = layout.inner_inset;
    ...
    const hy = rect.min[1] + (header_h - layout.font_size) * 0.5;
    const hdr_color = zcolors.withAlpha(self.theme.colors.text, 160);
    ...
    if (self.ws.nodes.items.len == 0) {
        self.drawText(
            ...
            zcolors.withAlpha(self.theme.colors.text, 100),
        );
        return;
    }
    ...
    for (self.ws.nodes.items, 0..) |node, idx| {
        ...
        const dot_size = layout.font_size * 0.7;
        self.drawFilledRect(Rect.fromXYWH(tx + (col_status_w - inset - dot_size) * 0.5, ty + (layout.font_size - dot_size) * 0.5, dot_size, dot_size), dot_color);

        // Name
        self.drawTextTrimmed(tx + col_status_w, ty, col_name_w - inset, node.node_name, self.theme.colors.text);

        // ID (dimmed)
        self.drawTextTrimmed(tx + col_status_w + col_name_w, ty, col_id_w - inset, node.node_id, zcolors.withAlpha(self.theme.colors.text, 140));
        ...
        self.drawTextTrimmed(tx + col_status_w + col_name_w + col_id_w, ty, col_seen_w - inset, age_str, zcolors.withAlpha(self.theme.colors.text, 160));

        y += row_h + row_gap;
    }
}

pub fn drawNodeTreeView(self: anytype, rect: anytype) void {
    const layout = self.panelLayoutMetrics();
    const inset = layout.inner_inset;
    const row_h = layout.row_height;
    const row_gap = @max(1.0, layout.inner_inset * 0.15);
    ...
    const ws_header_color = zcolors.withAlpha(self.theme.colors.text, 180);
    self.drawText(rect.min[0] + inset, y + (row_h - layout.font_size) * 0.5, ws_name, ws_header_color);
    y += row_h + row_gap;

    // Track which node IDs appear in mounts so we can show unmounted nodes separately
    var mounted_node_ids = std.BoundedArray([]const u8, 64){};
    ...
    if (self.ws.selected_workspace_detail) |*detail| {
        for (detail.mounts.items) |*mount| {
            ...
            const dot_size = layout.font_size * 0.6;
            ...
            self.drawTextTrimmed(tx + dot_size + 6.0 * self.ui_scale, ty, rect.max[0] - tx - dot_size - inset, line, self.theme.colors.text);
            mounted_node_ids.append(mount.node_id) catch {};
            y += row_h + row_gap;
        }
    }
    ...
    for (self.ws.nodes.items) |node| {
        ...
        if (!is_mounted) {
            if (!has_unmounted) {
                ...
                y += row_gap * 3.0;
                self.drawText(rect.min[0] + inset, y + (row_h - layout.font_size) * 0.5, "Unmounted nodes:", zcolors.withAlpha(self.theme.colors.text, 140));
                y += row_h + row_gap;
                has_unmounted = true;
            }
            if (y + row_h > rect.max[1]) break;
