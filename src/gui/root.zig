...
const App = struct {
    ...
    fn pollWebSocket(self: *App) !void {
        std.log.debug("pollWebSocket: entered", .{});
        if (self.ws_client) |*client| {
            std.log.debug("pollWebSocket: about to call client.read()", .{});
            const maybe_msg = client.read() catch |err| switch (err) {
                error.WouldBlock => return,
                error.ConnectionClosed, error.Closed => {
                    std.log.debug("pollWebSocket: websocket connection closed while polling", .{});
                    ...
                    std.log.debug("pollWebSocket: websocket read error: {s}", .{@errorName(err)});
            ...
            std.log.debug("pollWebSocket: client.read() returned", .{});

            if (maybe_msg) |msg| {
                std.log.debug("pollWebSocket: received message (len={d})", .{msg.len});
                defer self.allocator.free(msg);
                try self.appendMessage("assistant", msg);
                std.log.debug("pollWebSocket: processed message", .{});
            } else {
                std.log.debug("pollWebSocket: no message available", .{});
            }
        }
    }
    ...
    fn tryConnect(self: *App) !void {
        ...
        std.log.debug("tryConnect: connected; clearing sessions and adding main session", .{});
        self.clearSessions();
    ...
    fn sendChatMessageText(self: *App, text: []const u8) !void {
        ...
        std.log.debug(
            "sendChatMessageText: user send requested (len={d}, connected={})",
            .{ text.len, self.ws_client != null },
        );
    ...
    fn appendMessage(self: *App, role: []const u8, content: []const u8) !void {
        std.log.debug(
            "appendMessage: adding role={s} content_len={d} (before_total={d})",
            .{ role, content.len, self.messages.items.len },
        );
        ...
        std.log.debug(
            "appendMessage: added role={s} id={s} (after_total={d})",
            .{ role, id, self.messages.items.len },
        );
    }
