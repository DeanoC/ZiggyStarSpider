...
- `Spiderweb` (server/control/reconciler/FUSE topology source)
...
- `[x]` ZSS project/node/workspace-status CLI commands.
...
### [x] PR-S1 (Spiderweb): `control.workspace_status` desired/actual/drift payload
...
### [x] PR-S2 (Spiderweb): background reconcile loop
...
### [x] PR-S3 (Spiderweb): event-driven reconcile triggers
...
### [x] PR-S4 (Spiderweb): lease-aware failover mount selection
- Prefer healthy node export for shared mount paths.
- Preserve mount path continuity during failover.

### [x] PR-S5 (Spiderweb): reconcile diagnostics API
...
### [x] PR-S6 (Spiderweb): high-level project bootstrap control op
...
### [x] PR-S7 (Spiderweb): persistent chat/capability job index
...
### [x] PR-S10 (Spiderweb): distributed workspace integration matrix
