...
fn linkFromSource(b: *std.Build, step: *std.Build.Step.Compile, mod: *std.Build.Module, options: Options) !void {
    ...
    const lib_dawn = addLibraryCompat(
        b,
        "dawn",
        step.root_module.resolved_target.?,
        if (options.debug) .Debug else .ReleaseFast,
        options.shared_libs,
    );
...
fn downloadBinary(
    ...
    _ = is_windows;
    _ = version;
    return error.BinaryDownloadsUnsupported;
}
...
fn dirExists(path: []const u8) bool {
    var dir = openDirPath(path, .{}) catch return false;
    dir.close();
    return true;
}
...
fn isLinuxDesktopTarget(target: std.Target) bool {
    return isLinuxDesktopLike(target.os.tag) and !target.abi.isAndroid();
}
...
fn defineCMacro(step: *std.Build.Step.Compile, name: []const u8, value: []const u8) void {
    step.root_module.addCMacro(name, value);
}

pub fn appendFlags(step: *std.Build.Step.Compile, flags: *StringList, debug_symbols: bool, is_cpp: bool) !void {
    if (debug_symbols) try flags.append("-g1") else try flags.append("-g0");
    if (is_cpp) try flags.append("-std=c++17");
    if (isLinuxDesktopTarget(step.rootModuleTarget())) {
        defineCMacro(step, "DAWN_USE_X11", "1");
        defineCMacro(step, "DAWN_USE_WAYLAND", "1");
    }
}
...
fn buildLibDawnCommon(b: *std.Build, step: *std.Build.Step.Compile, options: Options) !*std.Build.Step.Compile {
    const target = step.rootModuleTarget();
    const lib = if (!options.separate_libs) step else addLibraryCompat(
        b,
        "dawn-common",
        step.root_module.resolved_target.?,
        if (options.debug) .Debug else .ReleaseFast,
    ...
    if (isLinuxDesktopTarget(target)) lib.linkLibrary(b.dependency("x11_headers", .{
    ...
    var cpp_sources = StringList.init(b.allocator);
    if (target.os.tag == .macos) {
    ...
    var cpp_flags = StringList.init(b.allocator);
...
fn defineDawnEnableBackend(step: *std.Build.Step.Compile, options: Options) void {
    defineCMacro(step, "DAWN_ENABLE_BACKEND_NULL", "1");
    // TODO: support the Direct3D 11 backend
    // if (options.d3d11.?) step.defineCMacro("DAWN_ENABLE_BACKEND_D3D11", "1");
    if (options.d3d12.?) defineCMacro(step, "DAWN_ENABLE_BACKEND_D3D12", "1");
    if (options.metal.?) defineCMacro(step, "DAWN_ENABLE_BACKEND_METAL", "1");
    if (options.vulkan.?) defineCMacro(step, "DAWN_ENABLE_BACKEND_VULKAN", "1");
    if (options.desktop_gl.?) {
        defineCMacro(step, "DAWN_ENABLE_BACKEND_OPENGL", "1");
        defineCMacro(step, "DAWN_ENABLE_BACKEND_DESKTOP_GL", "1");
    }
    if (options.opengl_es.?) {
        defineCMacro(step, "DAWN_ENABLE_BACKEND_OPENGL", "1");
        defineCMacro(step, "DAWN_ENABLE_BACKEND_OPENGLES", "1");
    }
}
...
fn buildLibDawnNative(b: *std.Build, step: *std.Build.Step.Compile, options: Options) !*std.Build.Step.Compile {
    ...
    if (options.vulkan.? and !target.abi.isAndroid()) lib.linkLibrary(b.dependency("vulkan_headers", .{
        .target = step.root_module.resolved_target.?,
        .optimize = lib.root_module.optimize.?,
    ...
    defineCMacro(lib, "__kernel_ptr_semantics", "");

    defineCMacro(lib, "_HRESULT_DEFINED", "");
    defineCMacro(lib, "HRESULT", "long");
    defineDawnEnableBackend(lib, options);

    // TODO(build-system): make these optional
    defineCMacro(lib, "TINT_BUILD_SPV_READER", "1");
    defineCMacro(lib, "TINT_BUILD_SPV_WRITER", "1");
    defineCMacro(lib, "TINT_BUILD_WGSL_READER", "1");
    defineCMacro(lib, "TINT_BUILD_WGSL_WRITER", "1");
    defineCMacro(lib, "TINT_BUILD_MSL_WRITER", "1");
    defineCMacro(lib, "TINT_BUILD_HLSL_WRITER", "1");
    defineCMacro(lib, "TINT_BUILD_GLSL_WRITER", "1");
    defineCMacro(lib, "DAWN_NO_WINDOWS_UI", "1");
    ...
    if (options.d3d12.?) {
        defineCMacro(lib, "DAWN_NO_WINDOWS_UI", "");
        defineCMacro(lib, "__EMULATE_UUID", "");
        defineCMacro(lib, "_CRT_SECURE_NO_WARNINGS", "");
        defineCMacro(lib, "WIN32_LEAN_AND_MEAN", "");
        defineCMacro(lib, "D3D10_ARBITRARY_HEADER_ORDERING", "");
        defineCMacro(lib, "NOMINMAX", "");
        try flags.appendSlice(&.{
    ...
    if (isLinuxDesktopTarget(target)) {
        inline for ([_][]const u8{
    ...
    if (options.vulkan.?) {
        ...
        if (isLinuxDesktopTarget(target)) {
            ...
            defineCMacro(lib, "DAWN_USE_SYNC_FDS", "1");
        }
    }
...
fn buildLibTint(b: *std.Build, step: *std.Build.Step.Compile, options: Options) !*std.Build.Step.Compile {
    ...
    defineCMacro(lib, "TINT_BUILD_HLSL_WRITER", "1");
    defineCMacro(lib, "TINT_BUILD_GLSL_WRITER", "1");
    defineCMacro(lib, "TINT_BUILD_SYNTAX_TREE_WRITER", "1");
...
fn buildLibAbseilCpp(b: *std.Build, step: *std.Build.Step.Compile, options: Options) !*std.Build.Step.Compile {
    ...
    defineCMacro(lib, "_FILE_OFFSET_BITS", "64");
    defineCMacro(lib, "_LARGEFILE64_SOURCE", "");
    ...
    if (target.os.tag == .windows) {
        defineCMacro(lib, "ABSL_FORCE_THREAD_IDENTITY_MODE", "2");
        defineCMacro(lib, "WIN32_LEAN_AND_MEAN", "");
        defineCMacro(lib, "D3D10_ARBITRARY_HEADER_ORDERING", "");
        defineCMacro(lib, "_CRT_SECURE_NO_WARNINGS", "");
        defineCMacro(lib, "NOMINMAX", "");
...
fn buildLibDxcompiler(b: *std.Build, step: *std.Build.Step.Compile, options: Options) !*std.Build.Step.Compile {
    ...
    defineCMacro(lib, "UNREFERENCED_PARAMETER(x)", "");
    defineCMacro(lib, "MSFT_SUPPORTS_CHILD_PROCESSES", "1");
    defineCMacro(lib, "HAVE_LIBPSAPI", "1");
    defineCMacro(lib, "HAVE_LIBSHELL32", "1");
    defineCMacro(lib, "LLVM_ON_WIN32", "1");
...
fn appendScannedSources(b: *std.Build, step: *std.Build.Step.Compile, args: struct {
    ...
    var sources = StringList.init(b.allocator);
...
fn scanSources(
    ...
    var dir = openDirPath(abs_dir, .{ .iterate = true }) catch |err| {
...
fn openDirPath(path: []const u8, options: std.fs.Dir.OpenOptions) !std.fs.Dir {
